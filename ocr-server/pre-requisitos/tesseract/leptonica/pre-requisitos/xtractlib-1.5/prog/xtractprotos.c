 ...
 *   And the function prototypes can be sent to stdout, written
 *   to a named file, or placed in-line withing allheaders.h.
 *
 *   The signature is:
 *
 ...
 *
 *   xtractprotos can run in leptonica to generate allheaders.h
 *   without leptprotos.h and with an empty allheaders.h.  Of course,
 *   the usual situation is to run it with a valid allheaders.h,
 *   which includes all the function prototypes.  To avoid including
 *   all this in the input for each file, cpp runs here with -DNO_PROTOS,
 *   so the prototypes are not included.  This is much faster.
 *
